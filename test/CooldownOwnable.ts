...
describe("CooldownOwnable contract", function () {
    ...
    beforeEach(async () => {
        testContract = (await ethers.deployContract(contractNames.CooldownOwnableExample, [
            COOLDOWN_TIME,
    ...
    it("fails when user try to set cooldown time", async function () {
        await expect(testContract.connect(user).setCooldownTime(COOLDOWN_TIME)).to.be.revertedWithCustomError(
            testContract,
    ...
    it("allow owner to set cooldown time", async function () {
        ...
        expect(await testContract.cooldownOf(await user.getAddress())).to.be.revertedWithCustomError(
            testContract,
    ...
    it("updates cooldown correctly", async function () {
        ...
        await testContract.connect(user).helloWorld();
        expect(await testContract.cooldownOf(await user.getAddress())).to.equal(TIMESTAMP + 1); // we add +1 because we cound the block generated when calling hello world
    });
});
